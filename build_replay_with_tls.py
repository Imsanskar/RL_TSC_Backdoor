...
def build_tls_str_for_step(tls_states, link_map, roads, incoming_roads):
    ...
    for road_id, road in roads.items():
        ...
        # CityFlow expects one char per lane.
        # We have data on SUMO lanes (typically 0..n_sumo-1). CityFlow may have more
        # virtual lanes than SUMO has physical ones. We mirror to all CityFlow lanes:
        # if no data for a lane, fall back to its inner-neighbor (or red).
        per_lane = []
        for cf_lane_idx in range(n):
            # SUMO lane index = invert CityFlow lane index
            sumo_lane = n - 1 - cf_lane_idx
            ch = states_for_road.get(sumo_lane)
            if ch is None:
                # fall back to the highest-priority signal among any from this road
                if states_for_road:
                    best = max(states_for_road.values(), key=state_priority)
                    ch = best
                else:
                    ch = 'r'
            per_lane.append(ch)
