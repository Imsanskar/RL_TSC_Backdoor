...
# ─── CITYFLOW REPLAY CONFIGURATION ──────────────────────────────────
self.replay_file_path = "data/output_data/replay/attack_1x1_replay.txt"
self.replay_data = []
# Dynamically calculate the center of the SUMO map to align with CityFlow
bounds = self.net_obj.getBBox() # Returns ((xmin, ymin), (xmax, ymax))
self.center_x = (bounds[0][0] + bounds[1][0]) / 2.0
self.center_y = (bounds[0][1] + bounds[1][1]) / 2.0
# ────────────────────────────────────────────────────────────────────
...
# ─── LIVE TRACKING CAPTURE SYSTEM FOR REPLAY ────────────────────────
step_vehicles = []
active_ids = self.eng.vehicle.getIDList()
for v_id in active_ids:
try:
x, y = self.eng.vehicle.getPosition(v_id)
sumo_angle = self.eng.vehicle.getAngle(v_id)
# Shift coordinates so the map is perfectly centered at 0,0
calibrated_x = x - self.center_x
calibrated_y = y - self.center_y
# Convert SUMO degrees to CityFlow radians
cf_angle_rad = (90.0 - sumo_angle) * (pi / 180.0)
if cf_angle_rad < 0:
cf_angle_rad += 2 * pi
v_entry = f"{calibrated_x:.2f} {calibrated_y:.2f} {cf_angle_rad:.4f} 0 0 0 5.0 2.0"
step_vehicles.append(v_entry)
except Exception:
continue
step_string = ",".join(step_vehicles) + ";"
self.replay_data.append(step_string)
# ────────────────────────────────────────────────────────────────────
self.run += 1
...
return
print(f"Exporting active tracking array data into layout files at: {self.replay_file_path}")
try:
os.makedirs(os.path.dirname(self.replay_file_path), exist_ok=True)
with open(self.replay_file_path, 'w') as f:
# Add 7 empty steps for CityFlow header padding
for _ in range(7):
f.write(";\n")
# Write the actual recorded movements
for step_str in self.replay_data:
f.write(step_str + "\n")
print("Export complete!")
except Exception as e:
print(f"Failed to generate custom replay target output file: {e}")
